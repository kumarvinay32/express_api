...
if (fs.existsSync(envFilePath)) {
    require("dotenv").config({ path: envFilePath, override: false });
}
...
(() => {
    try {
        ...
        const gitignoreBase = Buffer.from(
            ``,
            'base64'
        ).toString();
        checkWriteText(
        ...
        checkWriteText(
            path.join(rootDirectory, '.cloudignore'),
            gitignoreBase
        );

        checkWriteText(
            path.join(rootDirectory, 'readme.md'),
            Buffer.from(DEFAULT_DATA['readme'], 'base64').toString()
        );
        checkWriteText(
        ...
        checkWriteText(
            ...
            Buffer.from(DEFAULT_DATA['favicon'], 'base64').toString()
        );
