...
const _pluralUninflectedRe = new RegExp('^(?:' + [..._plural.uninflected, ..._uninflected].join('|') + ')$', 'i');
const _pluralIrregularRe   = new RegExp('(.*?(?:\\b|_))((?:' + Object.keys(_plural.irregular).join('|') + '))$', 'i');
const _pluralRules         = Object.entries(_plural.rules).map(([rule, repl]) => [new RegExp(rule, 'i'), repl, rule]);

const _singularUninflectedRe = new RegExp('^(?:' + [..._singular.uninflected, ..._uninflected].join('|') + ')$', 'i');
const _singularIrregularRe   = new RegExp('(.*?(?:\\b|_))((?:' + Object.keys(_singular.irregular).join('|') + '))$', 'i');
const _singularRules         = Object.entries(_singular.rules).map(([rule, repl]) => [new RegExp(rule, 'i'), repl, rule]);
...
class util extends dt {
     ...
     * @returns {object}
     */
    static obj_merge(...args) {
        let target = {};
        Object.assign(target, args[0] || {});
        let sources = [].slice.call(args, 1);
        ...
        for (const v of (mixed || '').replace(/,/g, ':').split(':')) {
            ...
            if (ip.length === 4) {
                ...
                ip.forEach(function (i) {
                    if (!/^\d{1,3}$/.test(i) || i < 0 || i > 255) { f = false; }
                });
                if (f) { ipv4 = v.trim(); }
            }
        }
    ...
    static pluralize(word) {
        let regs = (word || '').match(_pluralIrregularRe);
        if (regs) {
            return regs[1] + regs[2].substring(0, 1) + _plural.irregular[regs[2].toLowerCase()].substring(1);
        }
        if ((word || '').match(_pluralUninflectedRe)) {
            return word;
        }
        for (const [re, , rule] of _pluralRules) {
            if (word.match(re)) {
    ...
    static singularize(word) {
        let regs = (word || '').match(_singularIrregularRe);
        if (regs) {
            return regs[1] + regs[2].substring(0, 1) + _singular.irregular[regs[2].toLowerCase()].substring(1);
        }
        if ((word || '').match(_singularUninflectedRe)) {
            return word;
        }
        for (const [re, , rule] of _singularRules) {
            if (word.match(re)) {
    ...
    static preg_replace(rule, replacement, word) {
        ...
        // every $N reference is substituted; references beyond the captured
        // groups resolve to '' (PHP preg_replace semantics, which the rule
        // tables rely on — e.g. '$1$2eries' with a single capture group)
        let replaced_word = word.substr(0, regs.index) + replacement.replace(/\$(\d+)/g, (m, i) => regs[Number(i)] || '');
    ...
    static generate_password(PasswdLen, Type = '') {
        const CharStr = '0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz@_#!:+=';
    ...
    /**
     * Get a cryptographically secure random integer between a given range (inclusive).
