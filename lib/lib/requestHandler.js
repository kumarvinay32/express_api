...
module.exports = (req, res, next) => {
    ...
    res.send = function (body) {
        ...
        if (typeof body === "string") {
            ...
            // JSON scalars ('null', '123', 'true') are not envelopes — treat
            // them as payload instead of crashing/dropping them below
        ...
        // Guard the HTTP status: an app-level `code` outside the valid HTTP
        // range must not crash res.status(); the body keeps the original code.
        const httpStatus = (Number.isInteger(resp.code) && resp.code >= 100 && resp.code <= 599)
            ? resp.code
            : (resp.error ? 500 : 200);
