...
module.exports = (req) => {
    if (Object.hasOwnProperty.call(req, "config") && Object.hasOwnProperty.call(req.config, "lang") && req.config.lang) {
        ...
        const langFile = resolveModuleFile(path.join(req._lang_dir, req.app_lang));
        if (!langFile) {
            throw new ConfigError(`Language "${req.app_lang}" file not found. Path: ${path.join(req._lang_dir, `${req.app_lang}(${SUPPORTED_EXTENSIONS.join('|')})`)}`);
        }
        req._lng_messages = requireModule(langFile);
