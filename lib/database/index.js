...
const readDbModels = async (req, db_name) => {
    try {
        ...
        if (fs.existsSync(path.join(req._models_dir, db_name))) {
            const initModelsFile = resolveModuleFile(path.join(req._models_dir, db_name, 'init-models'));
            if (initModelsFile) {
                const models = requireModule(initModelsFile)(req._conn[db_name]);
                ...
                await Promise.all(files.map(async (file) => {
                    ...
                    if (file.indexOf('.') !== 0 && isModuleFile(file) && base !== 'index' && base !== 'init-models') {
                        const model = requireModule(path.join(req._models_dir, db_name, file))(req._conn[db_name], Sequelize.DataTypes);
