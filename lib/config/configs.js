...
module.exports = (req) => {
    ...
    const appConfigFile = resolveModuleFile(join(req._conf_dir, "appConfig"));
    if (!appConfigFile) {
        throw new ConfigError(`Config file not found. Path:"${join(req._conf_dir, `appConfig(${SUPPORTED_EXTENSIONS.join('|')})`)}". If the project is not set up yet, run "npx express-api init" to complete the setup.`);
    }
    requireModule(appConfigFile)(req);
