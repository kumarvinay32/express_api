...
process.on("message", async ({ activity = 'index', execFunction = 'main', payload = {}, headers = {} }) => {
    ...
    try {
        const activity_path = resolveModuleFile(path.join(req._activity_dir, activity));
        if (!activity_path) {
            throw new Error(`Activity not found. Path: "${path.join(req._activity_dir, `${activity}(${SUPPORTED_EXTENSIONS.join('|')})`)}"`);
        }
        const activities = requireModule(activity_path);
